...
def _face( meterface, diameter, angle, needle_len, back_shade=255, needle_shade=0, thickness=1 ):
  ...
  # print(image)
  # print(meterface)
  image = tf.math.minimum( meterface, image )
  # image = tf.math.add_n([meterface, image])
  return image

def face_gen( meterimage, sub_dir, file_handle, diameter, angle_limit, needle_len, thickness=1 ):
    base_dir = os.getcwd()
...
def meterface_gen( image, dir_name, file_handle, diameter, needle_len_min=40, needle_len_max=50, thickness_max=1 ):
  for thickness in range(  1, thickness_max+1):
    for needle_len in range(  int( diameter*needle_len_min/100 ) , int( diameter*needle_len_max/100 ), 2):
     face_gen( image, dir_name, file_handle, diameter, 90, needle_len, thickness=thickness )


def meter_gen_csv( face, dir_name, csv_file, diameter, angle_limit, needle_len_min=70, needle_len_max=90, thickness_max=1 ):
    ...
    with open( os.path.join( dir_name, csv_file ), 'w') as file_handle:
        ...
        meterface_gen( image, dir_name, file_handle, diameter, thickness_max=thickness_max )
...
def main( argv ):
    ...
    try:
        ...
        print('python createMeterTensorImageNeedle.py -d <dir> -c <csv> -f <face>')
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print('python createMeterTensorImageNeedle.py -d <dir> -c <csv> -f <face>')
    ...
    print(" Directory ", dir  )
    meter_gen_csv( face, dir, csv, 144, 90, thickness_max=2 )
